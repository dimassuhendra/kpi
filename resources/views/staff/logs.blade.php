...
@section('content')
    <div class="container mx-auto px-4 md:px-0" x-data="{ editModal: false, activeCase: {} }">
        ...
        <div class="organic-card p-4 mb-6">
            <form action="{{ route('staff.kpi.logs') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
                ...
                <div class="w-full md:w-auto flex gap-2">
                    <button type="submit"
                        class="flex-1 md:flex-none bg-primary text-slate-900 px-6 py-2 rounded-lg font-bold text-sm hover:bg-emerald-400 transition">
                ...
                @forelse($logs as $log)
                    <tbody class="text-slate-300 border-t border-slate-700/50" x-data="{ expanded: false }">
                        <tr class="hover:bg-slate-800/20 transition-all cursor-pointer" @click="expanded = !expanded">
                            <td class="p-5 font-medium text-white">
                                <div class="flex items-center group">
                                    <i class="fas fa-chevron-right text-[10px] mr-3 transition-transform"
                                        :class="expanded ? 'rotate-90 text-primary' : ''"></i>
                            ...
                            <td class="p-5">
                                <span
                                    class="{{ $log->status == 'pending' ? 'text-amber-400 bg-amber-400/5 border-amber-400/20' : 'text-emerald-400 bg-emerald-400/5 border-emerald-400/20' }} text-[10px] px-3 py-1 rounded-full border uppercase font-bold tracking-tighter">
                                    ...
                                    @foreach ($log->details as $detail)
                                        <div class="bg-slate-800/40 p-5 rounded-xl border border-slate-700/50">
                                            <div class="flex justify-between items-start mb-3">
                                                <span
                                                    class="text-[9px] bg-slate-700 text-slate-300 px-2 py-0.5 rounded uppercase tracking-tighter">Case
                                                    #{{ $loop->iteration }}</span>
                                                @if ($log->status == 'pending')
                                                    <button
                                            ...
                                            <div
                                                ...
                                                <span><i class="far fa-clock mr-1 text-primary"></i>
                        ...
                        <div class="flex justify-between items-start mb-2" @click="expanded = !expanded">
                            <div>
                                <p class="text-[10px] text-slate-500 uppercase tracking-widest mb-1">
                                    {{ date('d M Y', strtotime($log->tanggal)) }}</p>
                            ...
                            <div class="flex flex-col items-end gap-2">
                                <span
                                    class="{{ $log->status == 'pending' ? 'text-amber-400 bg-amber-400/5 border-amber-400/20' : 'text-emerald-400 bg-emerald-400/5 border-emerald-400/20' }} text-[9px] px-2 py-0.5 rounded border uppercase font-bold">
                                    {{ $log->status == 'pending' ? 'Pending' : 'Done' }}
                                </span>
                                @if ($log->status == 'pending')
                                    <form action="{{ route('staff.kpi.destroy', $log->id) }}" method="POST" @click.stop>
                                        @csrf @method('DELETE')
                                        <button type="submit" class="text-rose-500 text-xs"><i
                                ...
                                <div class="bg-slate-800/40 p-4 rounded-xl border border-slate-700/50">
                                    ...
                                    <div class="flex flex-wrap gap-3 text-[9px] font-bold uppercase text-slate-400">
                                        @if ($log->user->divisi->nama_divisi == 'TAC')
                                            {{-- Tampilan Khusus TAC --}}
                                            <span><i class="far fa-clock text-primary"></i> {{ $detail->value_raw }}
                                                Menit</span>
                                            <span
                                                class="{{ $detail->is_mandiri ? 'text-emerald-400' : 'text-blue-400' }}">
                                                {{ $detail->is_mandiri ? 'Mandiri' : 'Bantuan' }}
                                            ...
                                            @if ($detail->temuan_sendiri)
                                                <span class="text-amber-400">Temuan</span>
                                            @endif
                                        @else
                                            {{-- Tampilan Khusus INFRA (Mengambil dari field tipe_kegiatan) --}}
                                            <span
        ...
        <div x-show="editModal" class="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4"
            ...
            <div
                class="bg-slate-900 border-t md:border border-slate-700 rounded-t-3xl md:rounded-2xl p-6 md:p-8 max-w-lg w-full z-50 shadow-2xl transition-transform overflow-y-auto max-h-[90vh]">
                ...
                <p class="text-slate-400 text-xs mb-6 uppercase tracking-widest">Lakukan perubahan pada rincian aktivitas
                </p>
                ...
                <form :action="'/staff/kpi/case-update/' + activeCase.id" method="POST">
                    ...
                    <div class="flex flex-col md:flex-row justify-end gap-3 mt-8">
                        <button type="submit"
                            class="order-1 md:order-2 bg-primary text-slate-900 font-bold px-6 py-4 md:py-2 rounded-xl hover:bg-emerald-400 transition text-sm shadow-lg">
                            Update Case
                        </button>
                        <button type="button" @click="editModal = false"
                            class="order-2 md:order-1 px-5 py-4 md:py-2 text-slate-400 hover:text-white transition text-sm text-center">
                            Batal
                        </button>
                    </div>
                </form>
