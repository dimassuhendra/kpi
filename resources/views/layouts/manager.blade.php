...
<head>
    ...
    <style>
        ...
        body {
            ...
            color: var(--text-main);
            /* Diubah agar tidak flashing saat pindah halaman */
...
<body class="min-h-screen flex flex-col">
    <nav class="glass-nav sticky top-0 z-50 w-full px-6 lg:px-12">
        <div class="flex justify-between h-20 items-center">
            ...
            <div class="hidden md:flex items-center space-x-4">
                ...
                <div class="flex bg-black/30 p-1.5 rounded-full gap-2 mr-2 border border-white/10">
                    <button onclick="setTheme('default')" title="Tema Gelap"
                        class="w-4 h-4 rounded-full bg-[#6366f1] border border-white/20 hover:scale-125 transition-all"></button>
                    <button onclick="setTheme('tema-1')" title="Tema Merah"
                        class="w-4 h-4 rounded-full bg-[#C40C0C] border border-white/20 hover:scale-125 transition-all"></button>
                    <button onclick="setTheme('tema-2')" title="Tema Teal"
                        class="w-4 h-4 rounded-full bg-[#016B61] border border-white/20 hover:scale-125 transition-all"></button>
                    <button onclick="setTheme('tema-3')" title="Tema Earth"
                        class="w-4 h-4 rounded-full bg-[#504B38] border border-white/20 hover:scale-125 transition-all"></button>
                </div>

                <div class="flex flex-col items-end mr-2 text-right">
                    <span class="text-[9px] uppercase tracking-[0.2em] font-bold leading-none mb-1"
                        style="color: var(--accent)">Access Level: Manager</span>
                    <span class="text-sm font-header font-semibold text-white">{{ Auth::user()->name }}</span>
                </div>

                <div
                    class="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center border border-white/10 group">
                ...
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-10 h-10 flex items-center justify-center rounded-xl bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white transition-all duration-300">
        ...
        <div id="mobile-menu" class="lg:hidden">
            <div class="py-6 space-y-1 px-4">
                <a href="{{ route('manager.dashboard') }}"
                    class="flex items-center px-4 py-3 rounded-2xl text-white bg-white/10">
    ...
    <footer class="p-8 text-center" style="background: rgba(0,0,0,0.05)">
        <p class="text-[10px] font-medium italic uppercase tracking-widest opacity-60">
            &copy; 2026 <span style="color: var(--primary)" class="font-bold">MyBolo Console</span> • Manager
    ...
    <script>
        ...
        // Fungsi Ganti Tema (Tanpa Window.onload agar instant)
