<div class="organic-card p-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
            ...
            <p class="text-xs text-slate-500 italic uppercase tracking-tighter">Data analisis berdasarkan laporan yang
                disetujui</p>
        </div>
        <div class="flex bg-slate-900 rounded-2xl p-1 border border-white/5">
            <a href="{{ route('manager.dashboard', ['view_type' => 'all']) }}"
                class="px-5 py-2 text-[10px] font-bold uppercase rounded-xl transition-all {{ ($viewType ?? 'all') == 'all' ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-slate-300' }}">
                Division Summary
            </a>
            <a href="{{ route('manager.dashboard', ['view_type' => 'compare']) }}"
                class="px-5 py-2 text-[10px] font-bold uppercase rounded-xl transition-all {{ ($viewType ?? 'all') == 'compare' ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-slate-300' }}">
                Staff Comparison
            </a>
        </div>
    </div>
    ...
    @if (($viewType ?? 'all') == 'all')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-slate-900/40 p-5 rounded-[30px] border border-white/5 flex flex-col items-center">
                <h4 class="text-[9px] font-bold uppercase text-slate-500 mb-4 tracking-widest text-center">Source:
                    Temuan Sendiri vs Penugasan</h4>
            ...
            <div class="bg-slate-900/40 p-5 rounded-[30px] border border-white/5">
                <h4 class="text-[9px] font-bold uppercase text-slate-500 mb-4 tracking-widest text-center">Avg Response
                    Time (Limit: 15m)</h4>
        ...
        <script>
            ...
            new Chart(document.getElementById('donutInisiatif'), {
                ...
                data: {
                    ...
                    datasets: [{
                        data: [{{ $summaryData['proaktif'] ?? 0 }}, {{ $summaryData['penugasan'] ?? 0 }}],
            ...
            new Chart(document.getElementById('donutMandiri'), {
                ...
                data: {
                    ...
                    datasets: [{
                        data: [{{ $summaryData['mandiri'] ?? 0 }}, {{ $summaryData['bantuan'] ?? 0 }}],
            ...
            new Chart(document.getElementById('barResponseThreshold'), {
                ...
                data: {
                    labels: ['Team Average'],
                    datasets: [{
                        data: [{{ $summaryData['avg_time'] ?? 0 }}],
                        backgroundColor: '{{ ($summaryData['avg_time'] ?? 0) > 15 ? '#f43f5e' : '#3b82f6' }}',
                        borderRadius: 12,
                        barThickness: 60,
                    }]
                },
    ...
    @else
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="bg-slate-900/40 p-6 rounded-3xl border border-white/5">
                <h4 class="text-[10px] font-bold uppercase text-slate-500 mb-4 tracking-widest text-center">Total Cases
                </h4>
            ...
            <div class="bg-slate-900/40 p-6 rounded-3xl border border-white/5">
                <h4 class="text-[10px] font-bold uppercase text-slate-500 mb-4 tracking-widest text-center">Inisiatif
                    Count</h4>
                <canvas id="chartInisiatif"></canvas>
            </div>
            <div class="bg-slate-900/40 p-6 rounded-3xl border border-white/5">
                <h4 class="text-[10px] font-bold uppercase text-slate-500 mb-4 tracking-widest text-center">Mandiri
                    Count</h4>
        ...
        <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
        <script>
            const labels = @json($staffChartData->pluck('nama'));
            const baseOptions = {
                plugins: {
                    legend: {
                        display: false
